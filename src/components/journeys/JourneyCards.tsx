...
export function JourneyCards() {
  return <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
    {journeys.map((journey, index) => <Card key={journey.slug} className="overflow-hidden border-border shadow-none">
      ...
      <CardContent className="flex h-full flex-col gap-4"><p className="text-sm leading-relaxed text-muted-foreground">{journey.promise}</p><Button asChild variant="outline" className="mt-auto w-full"><Link href={`/topics/${journey.slug}`} prefetch={false}>Explore this question <ArrowRight className="size-4" aria-hidden="true" /></Link></Button></CardContent>
    </Card>)}
  </div>;
}
