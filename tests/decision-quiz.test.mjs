...
test("truthy invalid option IDs cannot produce a brief", () => {
  for (const question of QUIZ_QUESTIONS) {
    const invalid = { ...FULL_ANSWERS, [question.id]: "not-an-option" };
    assert.equal(isQuizComplete(invalid), false, question.id);
    assert.equal(buildDecisionBrief(invalid), null, question.id);
  }
});
...
test("brief never contains a numeric score or candidacy field", () => {
  ...
  assert.doesNotMatch(JSON.stringify(brief).toLowerCase(), /candidacy|"score"/);
});
...
test("fertility question is prioritized with a reason when fertility matters", () => {
  const brief = buildDecisionBrief(FULL_ANSWERS);
  const item = brief.prioritizedQuestions.find((entry) => entry.question.toLowerCase().includes("children"));
  assert.ok(item);
  assert.equal(brief.prioritizedQuestions[0].question, item.question);
  assert.match(item.reason, /fertility matters/i);
});

test("fertility question is not force-boosted when fertility is not a priority", () => {
  const brief = buildDecisionBrief({ ...FULL_ANSWERS, fertilityPriority: "not-priority" });
  const item = brief.prioritizedQuestions.find((entry) => entry.question.toLowerCase().includes("children"));
  assert.ok(item);
...
test("cost-unclear answers surface pricing pages as the next action", () => {
  assert.equal(buildDecisionBrief({ ...FULL_ANSWERS, costClarity: "no-idea" }).primaryAction.href, "/pricing");
  assert.equal(buildDecisionBrief({ ...FULL_ANSWERS, costClarity: "multiple-quotes" }).primaryAction.href, "/decision-guide");
});
...
test("quiz engine keeps answers in component memory and validates option IDs before advancing", () => {
  const component = readFileSync(new URL("../src/components/quiz/DecisionQuizEngine.tsx", import.meta.url), "utf8");
  assert.doesNotMatch(component, /fetch\(|localStorage\.|sessionStorage\.|sendBeacon|gtag\(|trackEvent\(|trackPageView\(|trackCtaClick\(|XMLHttpRequest/);
  assert.match(component, /window\.print\(\)/);
  assert.match(component, /Back/);
  assert.match(component, /const validCurrent = currentQuestion\?\.options\.some\(\(option\) => option\.id === currentAnswer\) \?\? false/);
  assert.match(component, /disabled=\{!validCurrent\}/);
  assert.match(component, /if \(!validCurrent\) return/);
});

test("retired quiz routes redirect instead of rendering a fake score, and stay live", () => {
  for (const file of ["../src/app/(isolated)/quiz/page.tsx", "../src/app/(isolated)/quiz/healthspan/advanced/page.tsx", "../src/app/(isolated)/quiz/healthspan/result/[shareId]/page.tsx"]) {
    const source = readFileSync(new URL(file, import.meta.url), "utf8");
    assert.match(source, /redirect\(/, `${file} should redirect`);
    assert.doesNotMatch(source, /QuizResult|QuizEngine|AdvancedQuizEngine|api\/quiz\/healthspan/, `${file} should not render or call the retired scoring UI/API`);
  }
});

test("the real quiz entry page renders the deterministic engine, not the retired scoring UI", () => {
  const source = readFileSync(new URL("../src/app/(isolated)/quiz/healthspan/page.tsx", import.meta.url), "utf8");
